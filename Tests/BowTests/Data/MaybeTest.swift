...
class MaybeTest: XCTestCase {
    ...
    var generator : (Int) -> Kind<MaybeF, Int> {
    ...
    func testApplicativeErrorLaws() {
        ApplicativeErrorLaws<MaybeF, Bow.Unit>.check(applicativeError: Maybe<Int>.monadError(), eq: Maybe.eq(Int.order), eqEither: Maybe.eq(Either.eq(UnitEq(), Int.order)), gen: { () } )
    }
    
    func testMonadErrorLaws() {
        MonadErrorLaws<MaybeF, Bow.Unit>.check(monadError: Maybe<Int>.monadError(), eq: self.eq, gen: { () })
    }
    
    func testSemigroupLaws() {
        property("Maybe semigroup laws") <- forAll { (a : Int, b : Int, c : Int) in
            return SemigroupLaws<Kind<MaybeF, Int>>.check(
    ...
    func testMonoidLaws() {
        property("Maybe monoid laws") <- forAll { (a : Int) in
            return MonoidLaws<Kind<MaybeF, Int>>.check(
