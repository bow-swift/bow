...
class ConstTest: XCTestCase {
    var generator : (Int) -> Kind<Kind<ForConst, Int>, Int> {
    ...
    func testFunctorLaws() {
        FunctorLaws<Kind<ForConst, Int>>.check(functor: Const<Int, Int>.functor(), generator: Const<Int, Int>.pure, eq: self.eq, eqUnit: self.eqUnit)
    }
    
    func testApplicativeLaws() {
        ApplicativeLaws<Kind<ForConst, Int>>.check(applicative: Const<Int, Int>.applicative(Int.sumMonoid), eq: self.eq)
    }
    
    func testSemigroupLaws() {
        property("Const semigroup laws") <- forAll { (a : Int, b : Int, c : Int) in
            return SemigroupLaws<Kind2<ForConst, Int, Int>>.check(
    ...
    func testMonoidLaws() {
        property("Const monoid laws") <- forAll { (a : Int) in
            return MonoidLaws<Kind2<ForConst, Int, Int>>.check(
