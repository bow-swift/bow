...
public typealias CofreePartial<S> = Kind<CofreeF, S>

public class Cofree<S, A> : Kind2<CofreeF, S, A> {
    private let head : A
    ...
    public static func fix(_ fa : Kind2<CofreeF, S, A>) -> Cofree<S, A> {
    ...
    public func cataM<B, M, FuncK, Trav, Mon>(_ folder : @escaping (A, Kind<S, B>) -> Kind<M, B>, _ inclusion : FuncK, _ traverse : Trav, _ monad : Mon) -> Kind<M, B> where FuncK : FunctionK, FuncK.F == EvalF, FuncK.G == M, Trav : Traverse, Trav.F == S, Mon : Monad, Mon.F == M {
...
public class CofreeFunctor<S, Func> : Functor where Func : Functor, Func.F == S {
    ...
    public func map<A, B>(_ fa: Kind<Kind<CofreeF, S>, A>, _ f: @escaping (A) -> B) -> Kind<Kind<CofreeF, S>, B> {
...
public class CofreeComonad<S, Func> : CofreeFunctor<S, Func>, Comonad where Func : Functor, Func.F == S {
    ...
    public func coflatMap<A, B>(_ fa: Kind<Kind<CofreeF, S>, A>, _ f: @escaping (Kind<Kind<CofreeF, S>, A>) -> B) -> Kind<Kind<CofreeF, S>, B> {
        return Cofree.fix(fa).coflatMap(f, functor)
    }
    
    public func extract<A>(_ fa: Kind<Kind<CofreeF, S>, A>) -> A {
