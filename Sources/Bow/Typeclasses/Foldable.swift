...
public extension Foldable {
    ...
    static func fold<A: Monoid>(_ fa: Kind<Self, A>) -> A {
        foldLeft(fa, A.empty(), { acc, a in acc.combine(a) })
    }
    ...
    static func reduceLeftToOption<A, B>(
        ...
        foldLeft(fa, Option.empty, { option, a in
            option^.fold(constant(Option.some(f(a))),
    ...
    static func reduceRightToOption<A, B>(
        ...
        foldRight(fa, Eval.now(Option.empty), { a, lb in
    ...
    static func foldMap<A, B: Monoid>(
        _ fa: Kind<Self, A>,
        _ f: @escaping (A) -> B) -> B {
        foldLeft(fa, B.empty(), { b, a in b.combine(f(a)) })
    }
    ...
    static func traverse_<G: Applicative, A, B>(
        ...
        foldRight(fa, Eval.always({ G.pure(unit) }), { a, acc in
    ...
    static func find<A>(
        ...
        foldRight(fa, Eval.now(Option.none()), { a, lb in
    ...
    static func exists<A>(
        ...
        foldRight(fa, Eval.false, { a, lb in
    ...
    static func forall<A>(
        _ fa: Kind<Self, A>,
        _ predicate: @escaping (A) -> Bool) -> Bool {
        foldRight(fa, Eval.true, { a, lb in
    ...
    static func isEmpty<A>(_ fa: Kind<Self, A>) -> Bool {
        foldRight(fa, Eval.true, { _, _ in Eval.false }).value()
    }
    ...
    static func foldM<G: Monad, A, B>(
        ...
        foldLeft(fa, G.pure(b), { gb, a in G.flatMap(gb, { b in f(b, a) }) })
    }
    ...
    static func foldMapM<G: Monad, A, B: Monoid>(
        ...
        foldM(fa, B.empty(), { b, a in
            G.map(f(a), { bb in b.combine(bb) })
        })
    }
...
public extension Kind where F: Foldable {
    ...
    func reduceRightToOption<B>(
        _ f: @escaping (A) -> B,
        _ g: @escaping (A, Eval<B>) -> Eval<B>) -> Eval<Option<B>> {
        F.reduceRightToOption(self, f, g)
    }
