...
public typealias ValidatedPartial<E> = Kind<ValidatedF, E>

public class Validated<E, A> : Kind2<ValidatedF, E, A> {
    ...
    public static func fix(_ fa : Kind2<ValidatedF, E, A>) -> Validated<E, A> {
    ...
    public func traverse<G, B, Appl>(_ f : (A) -> Kind<G, B>, _ applicative : Appl) -> Kind<G, Kind2<ValidatedF, E, B>> where Appl : Applicative, Appl.F == G {
...
public class ValidatedFunctor<R> : Functor {
    ...
    public func map<A, B>(_ fa: Kind<Kind<ValidatedF, R>, A>, _ f: @escaping (A) -> B) -> Kind<Kind<ValidatedF, R>, B> {
...
public class ValidatedApplicative<R, SemiG> : ValidatedFunctor<R>, Applicative where SemiG : Semigroup, SemiG.A == R {
    ...
    public func pure<A>(_ a: A) -> Kind<Kind<ValidatedF, R>, A> {
        return Validated<R, A>.valid(a)
    }
    
    public func ap<A, B>(_ fa: Kind<Kind<ValidatedF, R>, A>, _ ff: Kind<Kind<ValidatedF, R>, (A) -> B>) -> Kind<Kind<ValidatedF, R>, B> {
...
public class ValidatedApplicativeError<R, SemiG> : ValidatedApplicative<R, SemiG>, ApplicativeError where SemiG : Semigroup, SemiG.A == R {
    ...
    public func raiseError<A>(_ e: R) -> Kind<Kind<ValidatedF, R>, A> {
        return Validated<R, A>.invalid(e)
    }
    
    public func handleErrorWith<A>(_ fa: Kind<Kind<ValidatedF, R>, A>, _ f: @escaping (R) -> Kind<Kind<ValidatedF, R>, A>) -> Kind<Kind<ValidatedF, R>, A> {
...
public class ValidatedFoldable<R> : Foldable {
    ...
    public func foldL<A, B>(_ fa: Kind<Kind<ValidatedF, R>, A>, _ b: B, _ f: @escaping (B, A) -> B) -> B {
        return Validated.fix(fa).foldL(b, f)
    }
    
    public func foldR<A, B>(_ fa: Kind<Kind<ValidatedF, R>, A>, _ b: Eval<B>, _ f: @escaping (A, Eval<B>) -> Eval<B>) -> Eval<B> {
        return Validated.fix(fa).foldR(b, f)
    }
}

public class ValidatedTraverse<R> : ValidatedFoldable<R>, Traverse {
    public func traverse<G, A, B, Appl>(_ fa: Kind<Kind<ValidatedF, R>, A>, _ f: @escaping (A) -> Kind<G, B>, _ applicative: Appl) -> Kind<G, Kind<Kind<ValidatedF, R>, B>> where G == Appl.F, Appl : Applicative {
...
public class ValidatedSemigroupK<R, SemiG> : SemigroupK where SemiG : Semigroup, SemiG.A == R {
    ...
    public func combineK<A>(_ x: Kind<Kind<ValidatedF, R>, A>, _ y: Kind<Kind<ValidatedF, R>, A>) -> Kind<Kind<ValidatedF, R>, A> {
...
public class ValidatedEq<L, R, EqL, EqR> : Eq where EqL : Eq, EqL.A == L, EqR : Eq, EqR.A == R {
    public typealias A = Kind2<ValidatedF, L, R>
    private let eql : EqL
    private let eqr : EqR
    ...
    public func eqv(_ a: Kind2<ValidatedF, L, R>, _ b: Kind2<ValidatedF, L, R>) -> Bool {
