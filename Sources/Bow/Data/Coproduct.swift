...
public class Coproduct<F, G, A> : Kind3<ForCoproduct, F, G, A> {
    fileprivate let run : Either<Kind<F, A>, Kind<G, A>>
    
    public static func fix(_ fa : Kind3<ForCoproduct, F, G, A>) -> Coproduct<F, G, A> {
    ...
    public func traverse<B, H, Appl, TravF, TravG>(_ f : @escaping (A) -> Kind<H, B>, _ applicative : Appl, _ traverseF : TravF, _ traverseG : TravG) -> Kind<H, Kind3<ForCoproduct, F, G, B>> where Appl : Applicative, Appl.F == H, TravF : Traverse, TravF.F == F, TravG : Traverse, TravG.F == G {
...
public class CoproductFunctor<G, H, FuncG, FuncH> : Functor where FuncG : Functor, FuncG.F == G, FuncH : Functor, FuncH.F == H {
    ...
    public func map<A, B>(_ fa: Kind<Kind<Kind<ForCoproduct, G>, H>, A>, _ f: @escaping (A) -> B) -> Kind<Kind<Kind<ForCoproduct, G>, H>, B> {
...
public class CoproductComonad<G, H, ComonG, ComonH> : CoproductFunctor<G, H, ComonG, ComonH>, Comonad where ComonG : Comonad, ComonG.F == G, ComonH : Comonad, ComonH.F == H {
    ...
    public func coflatMap<A, B>(_ fa: Kind<Kind<Kind<ForCoproduct, G>, H>, A>, _ f: @escaping (Kind<Kind<Kind<ForCoproduct, G>, H>, A>) -> B) -> Kind<Kind<Kind<ForCoproduct, G>, H>, B> {
        return Coproduct.fix(fa).coflatMap(comonadG, comonadH, f)
    }
    
    public func extract<A>(_ fa: Kind<Kind<Kind<ForCoproduct, G>, H>, A>) -> A {
...
public class CoproductFoldable<G, H, FoldG, FoldH> : Foldable where FoldG : Foldable, FoldG.F == G, FoldH : Foldable, FoldH.F == H {
    ...
    public func foldL<A, B>(_ fa: Kind<Kind<Kind<ForCoproduct, G>, H>, A>, _ b: B, _ f: @escaping (B, A) -> B) -> B {
        return Coproduct.fix(fa).foldL(b, f, foldableG, foldableH)
    }
    
    public func foldR<A, B>(_ fa: Kind<Kind<Kind<ForCoproduct, G>, H>, A>, _ b: Eval<B>, _ f: @escaping (A, Eval<B>) -> Eval<B>) -> Eval<B> {
...
public class CoproductTraverse<G, H, TravG, TravH> : CoproductFoldable<G, H, TravG, TravH>, Traverse where TravG : Traverse, TravG.F == G, TravH : Traverse, TravH.F == H {
    ...
    public func traverse<I, A, B, Appl>(_ fa: Kind<Kind<Kind<ForCoproduct, G>, H>, A>, _ f: @escaping (A) -> Kind<I, B>, _ applicative: Appl) -> Kind<I, Kind<Kind<Kind<ForCoproduct, G>, H>, B>> where I == Appl.F, Appl : Applicative {
...
public class CoproductEq<F, G, B, EqB> : Eq where EqB : Eq, EqB.A == Kind2<ForEither, Kind<F, B>, Kind<G, B>>{
    public typealias A = Kind3<ForCoproduct, F, G, B>
    ...
    public func eqv(_ a: Kind<Kind<Kind<ForCoproduct, F>, G>, B>, _ b: Kind<Kind<Kind<ForCoproduct, F>, G>, B>) -> Bool {
