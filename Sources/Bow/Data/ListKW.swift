...
public class ForListKW {}

public class ListKW<A> : Kind<ForListKW, A> {
    ...
    private static func go<B>(_ buf : [B], _ f : (A) -> Kind<ForListKW, Either<A, B>>, _ v : ListKW<Either<A, B>>) -> [B] {
        if !v.isEmpty {
            let head = v.list[0]
    ...
    public static func tailRecM<B>(_ a : A, _ f : (A) -> Kind<ForListKW, Either<A, B>>) -> ListKW<B> {
        return ListKW<B>(go([], f, f(a).fix()))
    }
    
    public static func fix(_ fa : Kind<ForListKW, A>) -> ListKW<A> {
        return fa.fix()
    }
    ...
    public func traverse<G, B, Appl>(_ f : @escaping (A) -> Kind<G, B>, _ applicative : Appl) -> Kind<G, Kind<ForListKW, B>> where Appl : Applicative, Appl.F == G {
        let x = foldR(Eval.always({ applicative.pure(ListKW<B>([])) }),
                     { a, eval in applicative.map2Eval(f(a), eval, { x, y in ListKW<B>([x]) + y }) }).value()
        return applicative.map(x, { a in a as Kind<ForListKW, B> })
    }
...
public extension Kind where F == ForListKW {
...
public class ListKWFunctor : Functor {
    public typealias F = ForListKW
    
    public func map<A, B>(_ fa: Kind<ForListKW, A>, _ f: @escaping (A) -> B) -> Kind<ForListKW, B> {
        return fa.fix().map(f)
    }
}

public class ListKWApplicative : ListKWFunctor, Applicative {
    public func pure<A>(_ a: A) -> Kind<ForListKW, A> {
        return ListKW.pure(a)
    }
    
    public func ap<A, B>(_ fa: Kind<ForListKW, A>, _ ff: Kind<ForListKW, (A) -> B>) -> Kind<ForListKW, B> {
        return fa.fix().ap(ff.fix())
    }
}

public class ListKWMonad : ListKWApplicative, Monad {
    public func flatMap<A, B>(_ fa: Kind<ForListKW, A>, _ f: @escaping (A) -> Kind<ForListKW, B>) -> Kind<ForListKW, B> {
        return fa.fix().flatMap({ a in f(a).fix() })
    }
    
    public func tailRecM<A, B>(_ a: A, _ f: @escaping (A) -> Kind<ForListKW, Either<A, B>>) -> Kind<ForListKW, B> {
...
public class ListKWFoldable : Foldable {
    ...
    public func foldL<A, B>(_ fa: Kind<ForListKW, A>, _ b: B, _ f: @escaping (B, A) -> B) -> B {
        return fa.fix().foldL(b, f)
    }
    
    public func foldR<A, B>(_ fa: Kind<ForListKW, A>, _ b: Eval<B>, _ f: @escaping (A, Eval<B>) -> Eval<B>) -> Eval<B> {
        return fa.fix().foldR(b, f)
    }
}

public class ListKWTraverse : ListKWFoldable, Traverse {
    public func traverse<G, A, B, Appl>(_ fa: Kind<ForListKW, A>, _ f: @escaping (A) -> Kind<G, B>, _ applicative: Appl) -> Kind<G, Kind<ForListKW, B>> where G == Appl.F, Appl : Applicative {
...
public class ListKWSemigroupK : SemigroupK {
    ...
    public func combineK<A>(_ x: Kind<ForListKW, A>, _ y: Kind<ForListKW, A>) -> Kind<ForListKW, A> {
        return x.fix().combineK(y.fix())
    }
}

public class ListKWMonoidK : ListKWSemigroupK, MonoidK {
    public func emptyK<A>() -> Kind<ForListKW, A> {
        return ListKW<A>.empty()
    }
}

public class ListKWFunctorFilter : ListKWFunctor, FunctorFilter {
    public func mapFilter<A, B>(_ fa: Kind<ForListKW, A>, _ f: @escaping (A) -> Maybe<B>) -> Kind<ForListKW, B> {
        return fa.fix().mapFilter(f)
    }
}

public class ListKWMonadFilter : ListKWMonad, MonadFilter {
    public func empty<A>() -> Kind<ForListKW, A> {
        return ListKW<A>.empty()
    }
...
public class ListKWSemigroup<R> : Semigroup {
    public typealias A = Kind<ForListKW, R>
    
    public func combine(_ a: Kind<ForListKW, R>, _ b: Kind<ForListKW, R>) -> Kind<ForListKW, R> {
        return ListKW.fix(a) + ListKW.fix(b)
    }
}

public class ListKWMonoid<R> : ListKWSemigroup<R>, Monoid {
    public var empty: Kind<ForListKW, R> {
...
public class ListKWEq<R, EqR> : Eq where EqR : Eq, EqR.A == R {
    ...
    public func eqv(_ a: Kind<ForListKW, R>, _ b: Kind<ForListKW, R>) -> Bool {
        let a = ListKW.fix(a)
        let b = ListKW.fix(b)
