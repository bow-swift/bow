...
public protocol TraverseFilter : Traverse, FunctorFilter {
    func traverseFilter<A, B, G, Appl>(_ fa : HK<F, A>, _ f : (A) -> HK<G, Maybe<B>>, _ applicative : Appl) -> HK<G, HK<F, B>> where Appl : Applicative, Appl.F == G
}

public extension TraverseFilter {
    public func mapFilter<A, B>(_ fa: HK<F, A>, _ f: (A) -> Maybe<B>) -> HK<F, B> {
        return (traverseFilter(fa, { a in Id<Maybe<B>>.pure(f(a)) }, Id<Maybe<B>>.applicative()) as! Id<HK<F, B>>).extract()
    }
    
    public func filterA<A, G, Appl>(_ fa : HK<F, A>, _ f : (A) -> HK<G, Bool>, _ applicative : Appl) -> HK<G, HK<F, A>> where Appl : Applicative, Appl.F == G {
