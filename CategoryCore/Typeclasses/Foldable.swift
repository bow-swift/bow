...
public extension Foldable {
    public func fold<A, Mono>(_ monoid : Mono, _ fa : HK<F, A>) -> A where Mono : Monoid, Mono.A == A {
    ...
    public func reduceLeftMaybe<A>(_ fa : HK<F, A>, _ f : @escaping (A, A) -> A) -> Maybe<A> {
        return reduceLeftToMaybe(fa, id, f)
    }
    
    public func reduceRightMaybe<A>(_ fa : HK<F, A>, _ f : @escaping (A, Eval<A>) -> Eval<A>) -> Eval<Maybe<A>> {
        return reduceRightToMaybe(fa, id, f)
    }
    
    public func combineAll<A, Mono>(_ monoid : Mono, _ fa : HK<F, A>) -> A where Mono : Monoid, Mono.A == A {
        return fold(monoid, fa)
    }
    
    public func foldMap<A, B, Mono>(_ monoid : Mono, _ fa : HK<F, A>, _ f : @escaping (A) -> B) -> B where Mono : Monoid, Mono.A == B {
    ...
    public func sequence_<G, A, Appl>(_ applicative : Appl, _ fga : HK<F, HK<G, A>>) -> HK<G, Unit> where Appl : Applicative, Appl.F == G {
    ...
    public func foldM<G, A, B, Mon>(_ fa : HK<F, A>, _ b : B, _ f : @escaping (B, A) -> HK<G, B>, _ monad : Mon) -> HK<G, B> where Mon : Monad, Mon.F == G {
        return foldL(fa, monad.pure(b), { gb, a in monad.flatMap(gb, { b in f(b, a) }) })
    }
    
    public func foldMapM<G, A, B, Mon, Mono>(_ fa : HK<F, A>, _ f : @escaping (A) -> HK<G, B>, _ monad : Mon, _ monoid : Mono) -> HK<G, B> where Mon : Monad, Mon.F == G, Mono : Monoid, Mono.A == B {
        return foldM(fa, monoid.empty, { b, a in monad.map(f(a), { bb in monoid.combine(b, bb) }) }, monad)
    }
    
    public func get<A>(_ fa : HK<F, A>, _ index : Int64) -> Maybe<A> {
    ...
    public func size<A, Mono>(_ monoid : Mono, _ fa : HK<F, A>) -> Int64 where Mono : Monoid, Mono.A == Int64 {
