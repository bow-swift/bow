...
public protocol NonEmptyReducible : Reducible {
    ...
    func foldable() -> Fold
    func split<A>(_ fa : HK<F, A>) -> (A, HK<G, A>)
}
...
public extension NonEmptyReducible {
    ...
    public func foldR<A, B>(_ fa: HK<F, A>, _ b: Eval<B>, _ f: @escaping (A, Eval<B>) -> Eval<B>) -> Eval<B> {
    ...
    public func reduceRightTo<A, B>(_ fa: HK<F, A>, _ f: @escaping (A) -> B, _ g: @escaping (A, Eval<B>) -> Eval<B>) -> Eval<B> {
    ...
    public func foldM<G, A, B, Mon>(_ fa: HK<F, A>, _ b: B, _ f: @escaping (B, A) -> HK<G, B>, _ monad: Mon) -> HK<G, B> where G == Mon.F, Mon : Monad {
        let (a, ga) = split(fa)
