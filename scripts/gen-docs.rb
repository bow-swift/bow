...
def generate_nef_site(version, versions_list)
  ...
  `mkdir -p #{$source_dir}/#{version}/_data`
  File.write("#{$source_dir}/#{version}/_data/versions.json", JSON.pretty_generate(this_versions))
...
if !$current_branch_path.to_s.empty?
  `git checkout #{current_branch_tag}`
...
# Let's generate the versions file for the default version
