...
def generate_nef_site(title, version, versions_list)
  ...
  system "JEKYLL_ENV=production BUNDLE_GEMFILE=./#{$source_dir}/Gemfile bundle exec jekyll build -s ./#{$source_dir} -d ./#{$gen_docs_dir}/#{title} -b #{title}"
  system "rm -rf #{$source_dir}/docs"
  system "ls -la #{$gen_docs_dir}/#{title}"
end
